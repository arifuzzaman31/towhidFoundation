<section class="hero-area" data-background="assets/img/Background/header-bg.png">
<!--     <div class="hero-area-shape-1">
        <img src="assets/img/shape/hero-shape-1.png" alt="@lang('hero_area.tf')">
    </div> -->
    <div class="container">
        <div class="row justify-content-xl-center ">
            <div class="col-xl-7 col-lg-6">
                <div class="hero-inner-content ">
                    <h1>@lang('hero_area.tf')</h1>
                    <h4>@lang('hero_area.tf_sentence')</h4>
                    <div class="heuse-hero-btn mt-20">
                        <a href="#contact" class="btn btn-filled ">@lang('hero_area.contact_us')</a>
                    </div>
                </div>
            </div>
            <div class="col-xl-5 col-lg-6">
                <div class="hero-area-shape ">
                    <img src="{{URL::to('assets/img/shape/hero-area.png')}} " alt="@lang('hero_area.tf')">
                </div>
            </div>
        </div>
    </div>
</section>